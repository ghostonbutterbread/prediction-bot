...
- `shadow_fee_aware_edge_floor` — starts from an existing stable BUY and rejects it unless its recorded decision-time edge clears a configured fee-aware net-edge floor. Disabled by default; it cannot create trades, change position sizing, or mutate accounting.
